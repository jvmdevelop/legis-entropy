package com.jvmd.documentservice.parser;
...
public class KzLawNerParser {
    ...
    private void extractLawNames(String text, NerExtractionResult result) {
        ...
        while (matcher.find()) result.lawNames.add(new Entity("LAW_NAME", matcher.group(1).trim(), matcher.group(1).trim()));
    }

    private void extractCodeNames(String text, NerExtractionResult result) {
        Matcher matcher = CODE_NAME_PATTERN.matcher(text);
        while (matcher.find()) result.codeNames.add(new Entity("CODE_NAME", matcher.group(0).trim(), matcher.group(0).trim()));
    }

    private void extractDates(String text, NerExtractionResult result) {
        Matcher matcher = DATE_PATTERN.matcher(text);
        while (matcher.find()) result.dates.add(new Entity("DATE", matcher.group(0), matcher.group(0)));
    }

    private void extractMoneyAmounts(String text, NerExtractionResult result) {
        Matcher matcher = MONEY_PATTERN.matcher(text);
        while (matcher.find()) result.moneyAmounts.add(new Entity("MONEY", matcher.group(0).trim(), matcher.group(0).trim()));
    }

    private void extractDocumentNumbers(String text, NerExtractionResult result) {
        Matcher matcher = DOCUMENT_NUMBER.matcher(text);
        while (matcher.find()) result.documentNumbers.add(new Entity("DOCUMENT_NUMBER", matcher.group(0).trim(), matcher.group(0).trim()));
    }
    ...
    private void extractPersonNames(String text, NerExtractionResult result) {
        ...
        while (matcher.find()) {
            ...
            if (seen.add(name) && !isCommonWord(name)) result.personNames.add(new Entity("PERSON", name, name));
        }
    }
    ...
    private void extractLegalTerms(String text, NerExtractionResult result) {
        ...
        for (String term : legalTerms) {
            ...
            if (p.matcher(text).find()) result.legalTerms.add(new Entity("LEGAL_TERM", term, term));
        }
    }

    private boolean isCommonWord(String word) {
        String[] common = {"Республика","Казахстан","Закона","Статья","Пункта","Подпункта"};
        for (String c : common) if (word.equalsIgnoreCase(c)) return true;
        return false;
    }
