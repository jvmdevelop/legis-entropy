...
public class UserDocumentService {
    ...
    public void deleteDocument(String documentId, String userId) {
        ...
        vectorStore.delete(filterExpression(document.getUserId(), document.getId()));
        documentStorageService.delete(document.getObjectName());
        document.markDeleted();
        userDocumentRepository.save(document);
    }

    public Page<DocumentStatusResponse> listUserDocuments(String userId, Pageable pageable) {
        if (!hasText(userId)) throw new IllegalArgumentException("userId is required.");
        return userDocumentRepository
                .findByUserIdAndStatusNot(userId, DocumentProcessingStatus.DELETED, pageable)
                .map(this::toStatus);
    }
    ...
    public void reindexDocument(String documentId, String userId) {
        ...
        if (document.getStatus() == DocumentProcessingStatus.INDEXING) {
            throw new IllegalStateException("Document is already being indexed.");
        }
        vectorStore.delete(filterExpression(document.getUserId(), document.getId()));
    ...
    public List<Document> searchInUserDocuments(String query, String userId, String documentId) {
        if (!hasText(query)) throw new IllegalArgumentException("query is required.");
        if (!hasText(userId)) throw new IllegalArgumentException("userId is required.");
        ...
        SearchRequest searchRequest = SearchRequest.builder()
                ...
                .filterExpression(documentFilterEnabled
        ...
        return vectorStore.similaritySearch(searchRequest)
                .stream()
                .map(doc -> addKeywordScore(doc, query))
    ...
    private UserDocument requireDocument(String documentId, String userId) {
        if (!hasText(documentId)) throw new IllegalArgumentException("documentId is required.");
        if (!hasText(userId)) throw new IllegalArgumentException("userId is required.");
        return userDocumentRepository.findByIdAndUserId(documentId, userId)
    ...
    private DocumentStatusResponse toStatus(UserDocument document) {
        return new DocumentStatusResponse(
                ...
                document.getStatus(), document.getChunkCount(),
    ...
    private Document addKeywordScore(Document document, String query) {
        document.getMetadata().put("keywordScore", keywordScore(document.getText(), query));
        return document;
    }
    ...
    private double combinedScore(Document document) {
        ...
        double keyword = keywordScore instanceof Number n ? n.doubleValue() : 0.0;
        return vectorScore + keyword;
    }

    private double keywordScore(String text, String query) {
        if (!hasText(text) || !hasText(query)) return 0.0;
        String normalizedText = text.toLowerCase(Locale.ROOT);
        double score = 0.0;
        for (String term : query.toLowerCase(Locale.ROOT).split("\\W+")) {
            if (term.length() >= 3 && normalizedText.contains(term)) score += 0.05;
        }
        return score;
    }
    ...
    private String filterExpression(String userId, String documentId) {
        return userFilterExpression(userId) + " && documentId == '" + escapeFilterValue(documentId) + "'";
    }

    private boolean hasText(String value) { return value != null && !value.isBlank(); }
    private String escapeFilterValue(String value) { return value.replace("'", "\\'"); }
}
