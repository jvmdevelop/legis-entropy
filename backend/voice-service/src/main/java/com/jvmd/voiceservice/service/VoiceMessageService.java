...
public class VoiceMessageService {
    ...
    public void processAsync(String voiceMessageId) {
        try {
            ...
            log.error("Voice processing failed for {}: {}", voiceMessageId, e.getMessage(), e);
    ...
    public Optional<VoiceMessageDTO> transcribeAndAnalyze(String id) {
        ...
        try (InputStream in = loadAudio(v.getObjectName())) {
            VoiceTranscriptionService.TranscriptionResult tr =
                    transcriptionService.transcribe(in, v.getContentType(), v.getLanguage());
            ...
            List<String> speakers = tr.segments().stream()
                    .map(VoiceTranscriptionService.Segment::speaker)
                    .filter(s -> s != null).distinct().toList();
            VoiceAnalysisService.Classification cls = analysisService.classify(tr.plainText(), speakers);
    ...
    public List<VoiceMessageDTO> listByUser(String userId) {
        return repository.findByUserIdOrderByCreatedAtDesc(userId).stream().map(VoiceMessageDTO::from).toList();
    }

    public List<VoiceMessageDTO> listByGraph(String graphId) {
        return repository.findByGraphIdOrderByCreatedAtDesc(graphId).stream().map(VoiceMessageDTO::from).toList();
    }

    public List<VoiceMessageDTO> listBySituation(String situationId) {
        return repository.findBySituationIdOrderByCreatedAtDesc(situationId).stream().map(VoiceMessageDTO::from).toList();
    }
        ...
        repository.findById(id).ifPresent(v -> {
            try {
                minioClient.removeObject(io.minio.RemoveObjectArgs.builder()
                        .bucket(minioProperties.getBucket()).object(v.getObjectName()).build());
            } catch (Exception e) {
                log.warn("Could not remove voice object {}: {}", v.getObjectName(), e.getMessage());
            }
            repository.delete(v);
        });
    }
    ...
    public byte[] download(String id) {
        VoiceMessage v = repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Voice message not found: " + id));
        try (InputStream in = loadAudio(v.getObjectName())) {
    ...
    private String storeAudio(String id, String userId, MultipartFile file) {
        try {
            ensureBucket();
            String safeName = file.getOriginalFilename() == null ? "audio.bin"
            ...
            try (InputStream in = file.getInputStream()) {
                minioClient.putObject(PutObjectArgs.builder()
                        .bucket(minioProperties.getBucket()).object(objectName)
                        .contentType(file.getContentType()).stream(in, file.getSize(), -1).build());
            }
            return objectName;
        } catch (Exception e) {
            throw new IllegalStateException("Could not store voice audio in MinIO", e);
        }
    }
    ...
    private InputStream loadAudio(String objectName) {
        try {
            ...
            log.warn("Could not load audio {} from MinIO: {}", objectName, e.getMessage());
    ...
    private String serializeSegments(List<VoiceTranscriptionService.Segment> segments) {
        if (segments == null || segments.isEmpty()) return "[]";
        return segments.stream()
                .map(s -> "{\"speaker\":\"" + jsonEscape(s.speaker()) + "\",\"startMs\":" + s.startMs()
                        + ",\"endMs\":" + s.endMs() + ",\"text\":\"" + jsonEscape(s.text()) + "\"}")
                .collect(Collectors.joining(",", "[", "]"));
    }

    private String serializeClassification(VoiceAnalysisService.Classification c) {
        if (c == null) return "{}";
        StringBuilder sb = new StringBuilder();
        sb.append("{\"kind\":\"").append(jsonEscape(c.kind())).append("\",");
        sb.append("\"severity\":\"").append(jsonEscape(c.severity())).append("\",");
        sb.append("\"summary\":\"").append(jsonEscape(c.summary())).append("\",");
        sb.append("\"quotes\":[");
        sb.append(c.quotes().stream().map(q -> "\"" + jsonEscape(q) + "\"").collect(Collectors.joining(",")));
        sb.append("],\"articles\":[");
        List<String> arts = new ArrayList<>();
        for (VoiceAnalysisService.SuggestedArticle a : c.articles()) {
            arts.add("{\"lawCode\":\"" + jsonEscape(a.lawCode()) + "\",\"number\":\"" + jsonEscape(a.number())
                    + "\",\"reason\":\"" + jsonEscape(a.reason()) + "\"}");
        }
        sb.append(String.join(",", arts)).append("]}");
        return sb.toString();
    }
