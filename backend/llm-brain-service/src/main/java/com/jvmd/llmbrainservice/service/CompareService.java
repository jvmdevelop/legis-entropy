...
public class CompareService {
    ...
    public BrainResponse compare(CompareRequest req) {
        String leftText = resolveText(req.leftKind(), req.leftId(), req.userId(), req.leftLabel());
        String rightText = resolveText(req.rightKind(), req.rightId(), req.userId(), req.rightLabel());

        String leftHeader = headerFor(req.leftKind(), req.leftLabel(), req.leftId());
        String rightHeader = headerFor(req.rightKind(), req.rightLabel(), req.rightId());
        ...
        log.info("Compare: A={} ({} chars) vs B={} ({} chars)", leftHeader, leftText.length(), rightHeader, rightText.length());
        return brainModelClient.answer(prompt, List.of());
    }

    private String resolveText(String kind, String id, String userId, String label) {
        ...
        try {
            if ("LAW".equalsIgnoreCase(kind)) {
                ...
                if (law != null) {
                    ...
                    if (law.title() != null) sb.append(law.title()).append("\n\n");
                    if (law.summary() != null) sb.append(law.summary());
            ...
            if ("DOCUMENT".equalsIgnoreCase(kind)) {
                ...
                String text = res.getOrDefault("text", res.getOrDefault("content", ""));
                if (text == null || text.isBlank()) return fallback(label, "текст документа пуст");
                return truncate(text);
            }
            return fallback(label, "неизвестный тип " + kind);
        } catch (Exception e) {
            log.warn("Failed to load text for {}:{}: {}", kind, id, e.getMessage());
            return fallback(label, "ошибка загрузки: " + e.getMessage());
        }
    }

    private LawInfo lookupLaw(String code) {
        try {
            var graph = graphServiceClient.getLawWithRelationships(code, "RK");
            if (graph != null && graph.law() != null) return graph.law();
        } catch (Exception e) {
            log.warn("getLawWithRelationships failed for {}: {} — falling back to search", code, e.getMessage());
        }
        try {
            var matches = graphServiceClient.searchLaws(code, "RK");
            if (!matches.isEmpty()) return matches.get(0);
        } catch (Exception e) {
            log.warn("searchLaws fallback also failed for {}: {}", code, e.getMessage());
        }
        return null;
    }
    ...
    private static String fallback(String label, String reason) {
        return ("[Текст недоступен" + (label == null ? "" : " для «" + label + "»") + ": " + reason + "]");
    }
