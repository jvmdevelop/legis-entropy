...
@FeignClient(name = "documents-manager-service")
public interface DmsClient {
    ...
    List<RetrievalChunkResponse> searchLawsByCountry(
            @PathVariable("country") String country,
            @RequestParam("query") String query);

    @GetMapping("/api/user-documents/search")
    List<RetrievalChunkResponse> searchUserDocuments(
            @RequestParam("query") String query,
            @RequestHeader("X-User-Id") String userId,
            @RequestParam(value = "documentId", required = false) String documentId);

    @GetMapping("/api/user-documents/{documentId}/text")
    Map<String, String> getDocumentText(
            @PathVariable("documentId") String documentId,
            @RequestHeader("X-User-Id") String userId);

    @GetMapping("/api/voice-messages/{voiceId}")
    Map<String, Object> getVoiceMessage(@PathVariable("voiceId") String voiceId);
}
