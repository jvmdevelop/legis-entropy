...
public class DocumentUploadService {
    ...
    public DocumentUploadResult processDocument(
        MultipartFile file,
        UUID workspaceId,
        UUID userId
    ) throws IOException {
        log.info(
            "Processing document upload: {} (workspace: {})",
        ...
        KzLawNerParser.NerExtractionResult nerResult =
            nerParser.extractNamedEntities(plainText);
        ...
        return DocumentUploadResult.builder()
            ...
            .message(
                "Document processed successfully. Found " +
                    nerResult.getTotalEntities() +
                    " legal entities."
            )
            .build();
    }
    ...
    private String extractText(MultipartFile file) throws IOException {
        if (file.getSize() > MAX_FILE_SIZE) {
            throw new IOException("File exceeds maximum size of 10MB");
        }
        ...
        if (contentType != null && contentType.contains("application/pdf")) {
            ...
            contentType != null &&
            (contentType.contains("text") || contentType.contains("plain"))
        ) {
            ...
            log.warn(
                "Unsupported file format: {}. Attempting raw text extraction.",
                contentType
            );
            return new String(file.getBytes());
        }
