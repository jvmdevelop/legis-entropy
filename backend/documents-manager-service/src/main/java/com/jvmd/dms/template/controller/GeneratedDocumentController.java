...
import com.jvmd.dms.template.service.DocumentExportService;
...
public class GeneratedDocumentController {
    ...
    public ResponseEntity<GeneratedDocumentDTO> get(@PathVariable String id) {
        return templateService.getGenerated(id).map(ResponseEntity::ok)
    ...
    public ResponseEntity<GeneratedDocumentDTO> updateBody(
        ...
        return templateService.updateBody(id, markdown)
                ...
                if ("docx".equalsIgnoreCase(format)) {
                    ...
                    return ResponseEntity.ok()
                            .header(HttpHeaders.CONTENT_DISPOSITION,
                                    ContentDisposition.attachment().filename(slug + ".docx").build().toString())
                    ...
                    return ResponseEntity.ok()
                            .header(HttpHeaders.CONTENT_DISPOSITION,
                                    ContentDisposition.attachment().filename(slug + ".pdf").build().toString())
    ...
    public ResponseEntity<GeneratedDocumentDTO> regenerate(
        ...
        return templateService.regenerate(id, uid)
                .map(d -> ResponseEntity.status(org.springframework.http.HttpStatus.CREATED).body(d))
                .orElse(ResponseEntity.notFound().build());
    }
