...
public class DocumentExportService {
    ...
    public byte[] toPdf(String title, String markdown) throws IOException {
        String html = wrapHtml(
            title,
            htmlRenderer.render(mdParser.parse(markdown))
        );
        ...
        for (
            ...
            if (node instanceof Heading h) {
                ...
                for (
                    ...
                    para.setIndentationLeft(480);
                    XWPFRun run = para.createRun();
                    run.setText((idx++) + ". " + extractText(item));
                }
    ...
    private void appendInlines(XWPFParagraph para, Node parent) {
        for (
            ...
            if (node instanceof Text t) {
                ...
                node instanceof HardLineBreak || node instanceof SoftLineBreak
            ) {
                para.createRun().addBreak();
            } else {
                appendInlines(para, node);
            }
        }
    }
