...
public class TemplateController {
    ...
    public List<TemplateDTO> list(
        ...
        if (visibleOnly && userId != null && !userId.isBlank()) return templateService.listVisibleToUser(userId);
        return templateService.listAll();
    }

    @GetMapping("/{id}")
    public ResponseEntity<TemplateDTO> get(@PathVariable String id) {
        return templateService.findById(id).map(ResponseEntity::ok).orElse(ResponseEntity.notFound().build());
    }

    @GetMapping("/by-code/{code}")
    public ResponseEntity<TemplateDTO> getByCode(@PathVariable String code) {
        return templateService.findByCode(code).map(ResponseEntity::ok).orElse(ResponseEntity.notFound().build());
    }
    ...
    public ResponseEntity<TemplateDTO> upload(
            ...
            @RequestParam(required = false) String kind) {
        String uid = userId != null ? userId : "anonymous";
        return ResponseEntity.status(HttpStatus.CREATED).body(templateService.uploadFromFile(file, uid, title, kind));
    }
